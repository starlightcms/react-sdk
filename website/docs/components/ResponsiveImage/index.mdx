...
O componente **ResponsiveImage** renderiza uma imagem a partir de uma URL ou de um arquivo
...
O componente ResponsiveImage aceita uma propriedade `alt`, que define o atributo de
...
A seção [Variações](#variations) acima detalha como o componente ResponsiveImage cria
...
As imagens renderizadas pelo componente ResponsiveImage carregam dinamicamente
...
ResponsiveImage renderiza duas imagens: uma para navegadores com JavaScript ativado, e
uma para navegadores com JavaScript desabilitado.

Para prevenir esse problema, você pode utilizar o componente `GlobalLazyloadStyles`,
...
Por padrão, o componente ResponsiveImage carrega imagens que estão fora da tela caso
